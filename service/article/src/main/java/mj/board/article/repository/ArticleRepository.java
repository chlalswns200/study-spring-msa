...
public interface ArticleRepository extends JpaRepository<Article, Long> {
    ...
    @Query(
            value = "select count(*) from (" +
                    "select article.article_id from article where board_id = :boardId limit :limit" +
                    ") as temp",
            nativeQuery = true
    )
    Long count(@Param("boardId") Long boardId, @Param("limit") Long limit);

    @Query(value = "select article.article_id, article.title, article.content, article.board_id, article.writer_id, " +
            "article.created_at, article.modified_at " +
            "from article " +
            "where board_id = :boardId "+
